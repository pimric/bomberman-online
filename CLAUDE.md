...
- Commandes tactiles : `body.touch` si `(pointer: coarse)` ou `?tactile=1` (test sur ordinateur). Croix `#touchPad` (axe dominant doigt/centre, glisser d'une flèche à l'autre, écrit `gameState.keys` comme le clavier), bouton `#touchBomb` (`placeBomb`), `#touchDetonate` visible si détonateur (`updateTouchControls`). Mise en page en grille : portrait = plateau / croix + bouton / fiches ; paysage = croix | plateau | boutons, en-tête compact et `#gameInfo` masqué.
